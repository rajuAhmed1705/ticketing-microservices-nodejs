...
export const actions = {
  ...
  async addEmpType({ commit }, employeetype) {
    let res = await this.$axios.post("/employee-management/employment-type",employeetype)
    if(res.status == 201){
    commit("ADD_EMPTYPE", employeetype);
    }
  },
  async removeEmpType({ commit }, id) {
    let res = await this.$axios.delete(`/employee-management/employment-type/${id}`)
    if(res.status == 200){
    commit("REMOVE_EMPTYPE", id);
    }
  },
  async updateEmpType({ commit },employeetype) {
    let res = await this.$axios.put(`/employee-management/employment-type/${employeetype.id}`,employeetype)
    if(res.status == 200){
